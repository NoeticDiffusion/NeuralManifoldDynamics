"""Gate runner for FAR-EXT-002E scope 0.2 source rematerialization."""
...
PREREG_PATH = (
    FAR_ROOT
    / "014_far_ext_002e_scope_0.2_source_rematerialization_prereg.md"
)
SCOPE_PATH = FAR_ROOT / "FAR-EXT-002E-SCOPE-0.2.json"
DEFAULT_RESULT_002D = (
    FAR_ROOT / "results" / "far_ext_002d" / "far_ext_002d.json"
)
PRIOR_RESULT_002E = FAR_ROOT / "results" / "far_ext_002e" / "far_ext_002e.json"
DEFAULT_OUTPUT = (
    FAR_ROOT
    / "results"
...
def _prereg_scope_hash(path: Path) -> str:
    ...
    if match is None:
        raise ValueError("far_ext_002e_0_2_prereg_scope_hash_missing")
...
def _render_report(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# FAR-EXT-002E scope 0.2 — source rematerialization",
        "",
        "## Gate status",
        "",
        "```text",
        ...
        "## Binding",
        "",
        f"- 0.1 result: `{payload.get('entry_far_ext_002e_0_1', {}).get('status')}`",
    ...
    for document in payload.get("source_objects", []):
        lines.append(
            f"- `{document.get('evidence_id')}`: `{document.get('status')}` — "
            f"{document.get('snapshot_path')}"
        )
    ...
    *,
    ...
    if output_json.exists() or output_report.exists():
        raise FileExistsError(
            "refusing_to_overwrite_far_ext_002e_scope_0_2_archive"
        )
    if not PRIOR_RESULT_002E.is_file():
        raise FileNotFoundError(
            f"missing_immutable_far_ext_002e_0_1_result:{PRIOR_RESULT_002E}"
        )
    if not prereg_path.is_file():
        raise FileNotFoundError(f"missing_far_ext_002e_0_2_prereg:{prereg_path}")
    if not scope_path.is_file():
        raise FileNotFoundError(f"missing_far_ext_002e_0_2_scope:{scope_path}")
    ...
    if actual_scope_hash != expected_scope_hash:
        raise ValueError("far_ext_002e_0_2_scope_hash_mismatch")
    scope, _ = load_scope(
        scope_path,
        expected_scope_version="0.2",
    )
    ...
    payload.update(
        {
            ...
            "prior_result_002e_0_1_path": str(PRIOR_RESULT_002E),
            "prior_result_002e_0_1_sha256": sha256_file(PRIOR_RESULT_002E),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run FAR-EXT-002E scope 0.2 source-only bridge"
    )
    ...
    parser.add_argument(
        ...
        action="store_false",
        help="skip frozen source reads (non-canonical dry run)",
    )
