...
- **`fs_out`** *(float)*: configuration hint for consumers. It is not the derivative or Jacobian time base. See `fs_out_role` and `dt_realized_sec`.
- **`fs_out_role`** *(str)*: `configuration_hint` on exports written after P5.
- **`dt_realized_sec`** *(float)*: step in seconds used for derivatives and `J_dot`. On the global MNPS path this is the median `t_start` difference when that grid exists, otherwise `window_sec * (1 - overlap)`.
- **`super_window_requested`** *(int)*, **`super_window_realized`** *(int|None)*: configured Jacobian super-window and the odd length actually used. An even request is realized as request + 1.
...
- **`mndm_version`** *(str)*: software/measurement-contract release line recorded in the export metadata; current documentation tracks `3.0.2`.
...
- **`coverage_seconds_{assumed,effective,measured}`**, **`coverage_seconds_method`**, **`coverage_min_seconds_effective`**, **`coverage_min_epochs_effective`**, **`coverage_rule_tag`**: coverage-threshold provenance for this export. `coverage_seconds_measured` is the union of `[t_start, t_end]` intervals (`coverage_seconds_method=timestamps_union`); a hole in the middle is not usable duration. Fallback without timestamps is `len * dt` (`assumed_len_dt`). Derivatives split when `Δt_start > step * (1 + 0.25)`; samples adjacent to a time hole are NaN in `/mnps_3d_dot`.
...
- **`normalize_mode`**, **`export_contract_version`**: export-contract identifiers. fMRI recordings written after P0.2 use `mndm.fmri_h5_contract.v1`. EEG/MEG use `mndm.eeg_h5_contract.v1`. Older fMRI files can still carry `mndm.eeg_h5_contract.v1` because that string was the only contract name at the time. The historical name does not mean the recording is EEG; check `modality` and the source class (`nifti_bold`). Do not reinterpret an old fMRI file as an EEG contract solely from that attr.
- **`tr_sec`**, **`sfreq`**, **`tr_source`**: fMRI repetition-time provenance when present. `tr_source` is one of `bids_json`, `nifti_zooms`, `config_fallback`. JSON vs NIfTI mismatch above 1 ms is not serialized; ingest fails closed (`NOT_TESTABLE`). EEG exports omit these attrs.
- **`filter_stage`**, **`filter_applied`**, **`filter_bandpass_low`**, **`filter_bandpass_high`**: fMRI session-bandpass provenance when present. Canonical stage is `fmri_continuous` (once, before epoching). `preprocess_fmri` does not apply a second filter. EEG exports omit these attrs.
- **`nuisance_status`**: fMRI nuisance-regression provenance when present. One of `applied`, `skipped_missing_file`, `skipped_no_columns`, `truncated_length`, `failed_clean`, `disabled`. If `preprocess.fmri.nuisance_regression.enabled` is true, ingest fail-closes unless status is `applied`. Length mismatch is not silently truncated; remaining confound NaNs are not zero-filled. EEG exports omit this attr.
- **`atlas_space_status`**, **`atlas_ornt`**, **`bold_ornt`**, **`atlas_affine_match`**, **`atlas_affine_atol_mm`**: fMRI atlas vs BOLD spatial provenance when present. Same voxel shape is not the same space: mismatched affine/orientation is `NOT_TESTABLE` unless both `resample_atlas_to_bold` and `assume_same_space` are true. Affine comparison uses absolute tolerance `1e-3` mm (`rtol=0`); degenerate (near-singular) affines fail closed. fMRI summarize with `modality: fmri` fails closed if a single valid `atlas_space_status` (`matched` or `resampled`) is missing. EEG exports omit these attrs.
...
- **`row_source`** *(str)*: e.g. `"set_eeg"`, `"fif_meeg"`, `"nifti_bold"`, `"unknown"`.
- **`raw_file`** *(str)*: basename of the source file for each row.
- **`source_format`** *(str)*: `"neuromag_fif"`, `"eeglab_set"`, `"nifti_bold"`, `"unknown"`.
- **`has_meg`**, **`has_eeg`**, **`has_mag`**, **`has_grad`** *(int8, 0/1)*: per-row modality/channel-family presence flags. NIfTI BOLD rows have `has_eeg=0`.
...
The group does not carry atlas, TR, nuisance, or filter provenance. Those
fields are root attrs on the same file (`tr_sec`, `tr_source`, `sfreq`,
`filter_stage`, `nuisance_status`, `atlas_space_status`, `atlas_ornt`,
`bold_ornt`, `atlas_affine_match`). As of 2026-09-22 the only group attr
written by the H5 writer is `sfreq`.

- **`/regions/bold`** *(float32, shape `[n_regions, n_times]`)*: ROI×time matrix after parcellation. Non-finite voxels do not enter a label mean; an all-NaN ROI stays NaN. This is not a second bandpass.
- **`/regions/names`** *(utf-8 strings, shape `[n_regions]`)*: ROI names from the atlas label table, or `ROI_<id>` when a name is missing.
- **`/regions` attrs**:
  - **`sfreq`** *(float)*: sampling rate (Hz) for the `bold` time axis, equal to `1/tr_sec` from the resolved TR. Not `mnps.fs_out`.
...
- **`n_timepoints`** *(int)*
- **`dt_realized_sec`** *(float)*: sampling interval used for `/mnps_dot`. On the summarize path this is the same realized step as global `/mnps_3d_dot` (`t_start` median diff or `fmri_step_sec`).
- **`dt_source`** ∈ `{measured, config, default_1}`: `measured` from the recording grid; `config` from `time_step_sec`; `default_1` is the standalone fallback and is **not** a measured TR/step.
- **`derivative_method`**, **`derivative_window`**, **`derivative_polyorder`**: estimator settings inherited from `mnps.derivative` unless `regional_mnps.mnps.derivative` is set explicitly.

Missingness: regional MNPS is **not** median- or zero-filled before derivatives/Jacobians. An all-NaN or insufficiently finite network is omitted (`valid=False`). Sparse NaNs stay NaN; Jacobian is skipped rather than imputed. fMRI network PCs drop ROIs that are not fully finite instead of writing `0.0`.
