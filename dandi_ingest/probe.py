...
from dandi_ingest.contracts import AssetRecord, ProbeSummary
...
def probe_local_asset(record: AssetRecord, *, raw_root: Path) -> ProbeSummary:
    ...
    try:
        ...
        with NWBHDF5IO(str(local_path), "r", load_namespaces=True) as io:
            ...
            interval_tables: dict[str, object] = {}
            for interval_name, interval_table in getattr(nwbfile, "intervals", {}).items():
                try:
                    interval_tables[str(interval_name)] = {
                        "columns": [str(c) for c in getattr(interval_table, "colnames", ())],
                        "row_count": int(len(interval_table)),
                    }
                except Exception:
                    interval_tables[str(interval_name)] = {"error": "probe_failed"}
            if interval_tables:
                metadata["interval_tables"] = interval_tables
