...
from dandi_ingest.contracts import (
...
def _coerce_config(data: dict[str, Any]) -> DandiIngestionConfig:
    ...
    manifest_root = Path(str(storage_block.get("manifest_root") or output_root / "manifests" / dandiset_id))
    ...
    manifest_csv = Path(str(outputs_block.get("manifest_csv") or manifest_root / f"{config_id}_manifest.csv"))
    ...
    return DandiIngestionConfig(
        ...
        execution=execution,
        outputs=outputs,
        metadata={k: v for k, v in data.items() if k not in {"dataset", "storage", "selection", "execution", "outputs"}},
    )
